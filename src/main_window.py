...
from PyQt6.QtWidgets import (
    ...
    QMenu, QMenuBar, QTabWidget, QFrame, QFileDialog
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Message tracking
        self.receive_messages: Dict[int, dict] = {}  # id -> {msg, count, first_time, last_time}
    ...
    def _setup_ui(self):
        ...
        self.receive_table.setColumnCount(6)
        self.receive_table.setHorizontalHeaderLabels([
            "CAN-ID", "Type", "Length", "Data", "Cycle Time", "Count"
        ])
        self.receive_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.receive_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Fixed)
        self.receive_table.setColumnWidth(0, 100)
        self.receive_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Fixed)
        self.receive_table.setColumnWidth(1, 60)
        self.receive_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Fixed)
        self.receive_table.setColumnWidth(2, 60)
        ...
        self.periodic_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Fixed)
        self.periodic_table.setColumnWidth(0, 100)
        self.periodic_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Fixed)
        self.periodic_table.setColumnWidth(1, 60)
        self.periodic_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Fixed)
        self.periodic_table.setColumnWidth(2, 50)
        ...
        self.rules_table.setColumnWidth(0, 100)
    ...
    def _update_receive_table(self):
        ...
        for msg_id, entry in self.receive_messages.items():
            if self.filter_text:
                ...
                if self.filter_text not in id_hex:
                    continue
        ...
        for row, (msg_id, entry) in enumerate(sorted(filtered_messages.items())):
            ...
            self.receive_table.setItem(row, 1, type_item)
            
            # Length
            length_item = QTableWidgetItem(str(len(msg.data)))
            length_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.receive_table.setItem(row, 2, length_item)
            
            # Data
            data_str = " ".join(f"{b:02X}" for b in msg.data)
            data_item = QTableWidgetItem(data_str)
            self.receive_table.setItem(row, 3, data_item)
            
            # Cycle Time (calculated from count and time span)
            if entry['count'] > 1:
            ...
            self.receive_table.setItem(row, 4, cycle_item)
            
            # Count
            count_item = QTableWidgetItem(str(entry['count']))
            count_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.receive_table.setItem(row, 5, count_item)
